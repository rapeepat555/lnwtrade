    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-6 right-6 z-[300] bg-[#14161A] border border-[#10B981]/40 text-white px-4 py-3 rounded-2xl shadow-2xl flex items-center gap-3 backdrop-blur-md"
          >
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#636A78] mt-1 text-sm">Manage your trading funds, archives, and capital allocation</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-3">
          {/* Active / Archived Tab Filter Switch */}
          <div className="flex items-center bg-[#0A0B0E] p-1 rounded-xl border border-[#1F2228]">
            <button
            ...
            >
              ...
              <span>จัดเก็บแล้ว (Archived)</span>
          ...
          {!readOnly && (
            <button 
              ...
              className="flex items-center justify-center gap-2 bg-[#10B981] text-[#0A0B0E] hover:bg-[#10B981]/90 px-4 py-2.5 rounded-xl transition-all font-bold text-xs tracking-tight shadow-md shadow-[#10B981]/15"
            >
              <Plus className="w-4 h-4 text-[#0A0B0E]" />
              NEW WALLET
            </button>
          )}
        </div>
      ...
      <div className="relative group/wallets">
        {displayedPortfolios.length === 0 ? (
          <div className="bg-[#14161A] border border-[#1F2228] rounded-2xl p-8 text-center my-2">
            <div className="w-12 h-12 rounded-2xl bg-[#0A0B0E] border border-[#1F2228] flex items-center justify-center mx-auto mb-3 text-[#636A78]">
              {walletFilter === 'archived' ? <FolderArchive className="w-6 h-6 text-amber-500/50" /> : <WalletIcon className="w-6 h-6" />}
            </div>
            <p className="text-sm font-bold text-white mb-1">
              {walletFilter === 'archived' ? 'ไม่มีกระเป๋าที่จัดเก็บไว้' : 'ยังไม่มีกระเป๋าเงิน'}
            </p>
            <p className="text-xs text-[#636A78] max-w-sm mx-auto">
              {walletFilter === 'archived' 
          ...
          <div className="flex overflow-x-auto gap-4 pb-6 snap-x snap-mandatory no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
            {displayedPortfolios.map(p => (
              <div 
                ...
                className={cn(
                  "min-w-[290px] sm:min-w-[330px] flex-shrink-0 p-5 rounded-2xl border transition-all cursor-pointer group relative snap-center",
                  activePortfolioId === p.id 
                    ? (p.isArchived ? "bg-[#14161A] border-amber-500/80 shadow-lg shadow-amber-500/5" : "bg-[#14161A] border-[#10B981] shadow-lg shadow-[#10B981]/5")
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <div className={cn(
                      ...
                      {p.isArchived ? <FolderArchive className="w-4 h-4" /> : <CardIcon className="w-4 h-4" />}
                    </div>
                    {p.isArchived && (
                      ...
                      {p.isArchived ? (
                        ...
                        <>
                          <button 
                            ...
                            className="px-2.5 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-xl text-[#10B981] transition-all border border-emerald-500/20 shadow-sm flex items-center gap-1 text-[10px] font-bold"
                          ...
                          <button 
                            ...
                            className="p-2 bg-rose-500/10 hover:bg-rose-500/20 rounded-xl text-rose-500 transition-all border border-rose-500/20 shadow-sm"
                        ...
                        <>
                          ...
                          <button 
                            ...
                            className="p-2.5 bg-amber-500/10 hover:bg-amber-500/20 rounded-xl text-amber-400 transition-all border border-amber-500/20 shadow-sm"
      ...
      {activePortfolio && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-6">
            <div className={cn(
              "rounded-2xl p-8 text-white relative overflow-hidden border shadow-2xl transition-all",
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-2.5">
                    {activePortfolio.isArchived ? (
                      <FolderArchive className="text-amber-400 w-8 h-8" />
                    ) : (
                    ...
                    <div className="flex items-center gap-2">
                      {activePortfolio.isArchived ? (
                        <button 
                          ...
                          className="px-3 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-xl text-[#10B981] transition-all border border-emerald-500/20 shadow-sm flex items-center gap-1.5 text-xs font-bold"
                ...
                <div className="flex items-center justify-between">
                  <p className="text-[#636A78] text-[10px] font-bold uppercase tracking-widest mb-2">
                ...
                <div className="flex items-center gap-4 mt-8 pt-6 border-t border-[#1F2228]">
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase font-bold text-[#636A78]">Currency</span>
      ...
      {activePortfolio && (
        <div className="bg-[#14161A] p-4 sm:p-8 rounded-2xl border border-[#1F2228] shadow-sm">
          <div className="flex items-center justify-between mb-8">
            ...
            <p className="text-[10px] font-medium text-[#636A78] italic">Deposits & Withdrawals</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-[#0A0B0E] text-[#636A78] text-[10px] uppercase tracking-wider">
                <tr>
                  ...
                  {!readOnly && <th className="px-6 py-3 font-medium text-right">Action</th>}
                </tr>
              </thead>
              <tbody className="text-xs text-[#E0E0E0] divide-y divide-[#1F2228]">
                {(!activePortfolio.transactions || activePortfolio.transactions.length === 0) ? (
                  <tr>
                    <td colSpan={readOnly ? 3 : 4} className="px-6 py-8 text-center text-[#636A78] italic">
                  ...
                  activePortfolio.transactions.map((tx, idx) => {
                    ...
                    return (
                      <tr key={tx.id ? `tx-${tx.id}` : `tx-fallback-${idx}`} className="hover:bg-[#1F2228]/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-[#636A78] font-mono text-[10px]">
                        ...
                        {!readOnly && (
                          <td className="px-6 py-4 text-right">
                            <button
                              ...
                              className="p-1.5 hover:bg-rose-500/10 text-[#636A78] hover:text-rose-500 rounded-lg transition-colors"
